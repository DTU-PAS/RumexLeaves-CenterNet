...
PROJECT_NAME = rumexleaves_centernet
PYTHON_VERSION = 3.8
...
requirements:
	conda run -n rumexleaves_centernet $(PYTHON_INTERPRETER) -m pip install -U pip setuptools wheel
	conda run -n rumexleaves_centernet $(PYTHON_INTERPRETER) -m pip install -r requirements.txt
	conda run -n rumexleaves_centernet $(PYTHON_INTERPRETER) -m pip install -e .
	git submodule update --init --recursive
	conda run -n rumexleaves_centernet $(PYTHON_INTERPRETER) -m pip install -e submodules/Annotation-Converter/.
...
data:
ifeq (,$(wildcard data/raw/RumexLeaves.zip))
	wget -O data/raw/RumexLeaves.zip https://data.dtu.dk/ndownloader/files/41521812
endif
	python $(PROJECT_NAME)/data/make_dataset.py data/raw data/processed

download_weights:
ifeq (,$(wildcard models/final_model.pth))
	wget -O models/final_model.pth https://data.dtu.dk/ndownloader/files/44611330
endif
