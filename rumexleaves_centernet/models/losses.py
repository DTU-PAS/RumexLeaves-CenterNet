...
from rumexleaves_centernet.utils.model_utils import _transpose_and_gather_feat
...
class VeinStemLoss(nn.Module):
    ...
    def polar_to_cartesian(self, kp):
        ...
        if self.sincos:
            angle = torch.arctan2(((kp[:, :, :, 2] * 2) - 1), ((kp[:, :, :, 1] * 2) - 1))
        else:
            mult = torch.pi / 180
            if self.norm_target:
                mult *= 360
    ...
    def cartesian_to_polar(self, kp):
        ...
        if self.sincos:
            kp_polar = torch.zeros((kp.shape[0], kp.shape[1], 8, 3)).to(kp.device)
    ...
    def project_prediction(self, pred, target, mask):
        ...
        for b_i in range(batch):
            for i, (m, p, t) in enumerate(zip(mask[b_i], pred[b_i], target[b_i])):
                    ...
                    for j, (m_j, p_j, t_j) in enumerate(zip(m, p, t)):
                        if torch.sum(m_j) == 0:
                            ...
                            if j in true_kp_indices:
                                ...
                                projected_points[b_i, i, j, :] = self.get_closest_point_on_segment_list(p_j, a, b)
    ...
    def forward(self, output, mask, ind, target):
        ...
        target = target * mask
        with torch.no_grad():
            projected_points = self.project_prediction(pred.clone(), target, mask)
        loss = self.reg_loss(pred * mask, projected_points * mask, size_average=False)
