# Code taken from: https://github.com/Megvii-BaseDetection/YOLOX/blob/main/yolox/utils/logger.py
#!/usr/bin/env python3
...
class WandbLogger(object):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        from rumexleaves_centernet.data import RumexLeavesDataset
        ...
        self.voc_dataset = RumexLeavesDataset

        if val_dataset and self.num_log_images != 0:
            self.val_dataset = val_dataset
            self.cats = val_dataset.classes
            self.id_to_class = {id: cls for id, cls in enumerate(self.cats)}
        ...
        if self._run is None:
            if self.wandb.run is not None:
                ...
                if self.run_id is not None:
                    self.wandb.run.name = self.run_id
        return self._run
    ...
    def _convert_prediction_format(self, predictions):
        ...
        for i, prediction in enumerate(predictions):
            _, _, _, img_id = self.val_dataset.pull_item(i)
            ...
            image_wise_data.update(
                {
                    img_id: {
                        "bboxes": [box.detach().cpu().numpy().tolist() for box in act_box],
                        "scores": [score.detach().cpu().numpy().item() for score in act_scores],
                        "categories": [int(act_cls[ind]) for ind in range(len(act_box))],
                    }
                }
            )
    ...
    def log_images(self, predictions):
        ...
        for idx, val in table_ref.iterrows():
            ...
            average_class_score = []
            for cls in self.cats:
                if cls not in num_occurrences:
                    score = 0
                else:
                    score = avg_scores[cls] / num_occurrences[cls]
    ...
    def initialize_wandb_logger(cls, args, exp, val_dataset):
        ...
        return cls(
            ...
            run_id=exp.exp_name,
            project=os.path.basename(exp.output_dir),
            **wandb_params,
        )
